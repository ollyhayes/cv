...
* Created an interactive chart building framework built upon D3.js as a replacement for the expensive and now abandoned .net charting library. The framework has a focus on being extensively customable for clients requirements using a drop-in module system, and is now used for all their charting needs
...
Away from work for solo trip to asia.
...
* Designed and developed a high performance rastor-based mapping engine, featuring an interactive live tactical overlay, and the ability to pull mapping from various sources including Bing/OSM maps, remote government servers, and local storage for offline use. This is now used in all our GIS based products and has taken over from an MoD supplied mapping engine
...
* Build a spending tracker offline enabled web app that uploaded data to a google spreadsheet using their API
...
* Building a RaspberryPi powered web-based thermostat to control home heating (in progress)
